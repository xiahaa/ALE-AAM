...
mkdir -p "$PICK"
find "$PICK" -maxdepth 1 -type f -name 'silas_maptool-*.whl' -delete
set --
...
if ! "$PY" -m pip download --no-deps --no-index "$@" --dest "$PICK" silas-maptool; then
  ...
  echo "No compatible silas-maptool wheel was found for $PY_TAG on $PLATFORM." >&2
  echo "Download the matching prebuilt wheel and place it under dist/, then rerun install.sh." >&2
  echo "macOS artifacts are named silas-maptool-0.2.0-macos-wheels in the wheel CI." >&2
  exit 2
fi
WHEEL=$(find "$PICK" -maxdepth 1 -type f -name 'silas_maptool-*.whl' -print -quit)
if [ -z "$WHEEL" ]; then
  echo "Wheel selection completed without producing a silas-maptool wheel." >&2
  exit 2
fi
