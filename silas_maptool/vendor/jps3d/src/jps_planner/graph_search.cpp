...
bool GraphSearch::plan(StatePtr& currNode_ptr, int maxExpand, int start_id, int goal_id) {
  // Insert start node
  pq_.push({currNode_ptr->g + currNode_ptr->h, currNode_ptr->g,
      currNode_ptr->id, currNode_ptr});
  ...
  {
    expand_iteration++;
    // Get the current valid entry with the smallest cost.  Improved nodes are
    // inserted again, so entries carrying an older g snapshot are skipped.
    while(true) {
      if(pq_.empty())
        return false;
      const QueueEntry entry = pq_.top();
      pq_.pop();
      currNode_ptr = entry.state;
      if(!currNode_ptr->closed &&
          std::abs(entry.g - currNode_ptr->g) <= 0.000001)
        break;
    }
   ...
   // printf("size of succs: %zu\n", succ_ids.size());
    ...
    for( int s = 0; s < (int) succ_ids.size(); s++ )
    {
      ...
      if( tentative_gval < child_ptr->g && !child_ptr->closed )
      {
        ...
        if( child_ptr->opened && !child_ptr->closed) {
          ...
          pq_.push({child_ptr->g + child_ptr->h, child_ptr->g,
              child_ptr->id, child_ptr});
        }
