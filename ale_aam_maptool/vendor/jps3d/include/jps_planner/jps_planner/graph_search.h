...
namespace JPS
{
  ...
  /// Immutable heap entry.  The snapshots keep the std::priority_queue heap
  /// valid when a State's best-known g cost is improved later.
  struct QueueEntry
  {
    double f;
    double g;
  ...
  struct compare_queue_entry
  {
    bool operator()(const QueueEntry& a1, const QueueEntry& a2) const
    {
      if(std::abs(a1.f - a2.f) <= 0.000001) {
        if(std::abs(a1.g - a2.g) > 0.000001)
          return a1.g < a2.g;
        return a1.id > a2.id; // stable platform-independent final tie-break
      }
      return a1.f > a2.f;
    }
  };
  ...
  using priorityQueue = std::priority_queue<QueueEntry,
      std::vector<QueueEntry>, compare_queue_entry>;
