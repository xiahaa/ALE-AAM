...
export CIBW_TEST_COMMAND=${CIBW_TEST_COMMAND:-"python -m ale_aam_maptool doctor --json && ale-aam-maptool inspect --scenario {project}/sample_scenario --json && ale-aam-maptool plan-all --scenario {project}/sample_scenario --outdir {project}/wheel-test-output && python {project}/scripts/wheel_smoke.py {project}/sample_scenario {project}/wheel-test-output && python -m pytest {project}/tests"}
...
find "$OUT" -maxdepth 1 -type f -name 'ale_aam_maptool-*.whl' -print
