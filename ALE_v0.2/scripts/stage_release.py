...
def stage(output: Path, wheelhouse: Path) -> Path:
    ...
    native = [p for p in wheels if p.name.startswith("ale_aam_maptool-0.2.1-cp312-") and "linux" in p.name]
    if len(native) != 1:
        raise RuntimeError("wheelhouse must contain exactly one Linux CPython 3.12 ale-aam-maptool 0.2.1 wheel")
