"""Deterministically rebuild task metadata and documents for the three cases.

Authoritative GIS is already distributed in each task and is regenerated by
``refresh_official_sources.py`` from the planning extents declared here. This
script intentionally has no dependency on the historical ALE_v0.1 repository.
"""
...
TASKS = {
    "urban_drone_logistics": {
        ...
        "start": [114.1100062342439, 22.33649759809088],
        "goal": [114.14639844489409, 22.33705333134389],
        "bounds": [114.09060305, 22.31842728, 114.16581141, 22.35511694],
    ...
    "cross_sea_drone_logistics": {
        ...
        "start": [114.12953776982366, 22.260497414234752],
        "goal": [114.0286329722111, 22.208092486108427],
        "bounds": [114.00925789, 22.19001229, 114.148938, 22.2785632],
    ...
    "emergency_blood_transport": {
        ...
        "start": [114.15669210740508, 22.28314128069164],
        "goal": [114.17545103104615, 22.27613299225955],
        "bounds": [114.13729223, 22.25806688, 114.19486021, 22.30120278],
...
def task_json(name, cfg):
    return {
        ...
        "planning_extent":{"bounds_wgs84":cfg["bounds"],"corridor_buffer_m":2000,
...
def build():
    for name, cfg in TASKS.items():
        ...
        required = ("dem.tif", "buildings_3d.geojson", "airspace_zones.geojson",
                    "population_density.tif", "weather_grid.tif", "emergency_sites.geojson")
        missing = [filename for filename in required if not (gis / filename).is_file()]
        if missing:
            raise RuntimeError(f"{name}: missing authoritative GIS before metadata rebuild: {missing}")
